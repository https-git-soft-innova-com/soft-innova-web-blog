...
## Soft-Innova (este clon)

- **Node.js** `>=22.12.0` (Astro 6; si `npm run build` falla, sube la versión de Node).
- **Local:** `npm run dev` → admin en `/_emdash/admin`.
- **Datos demo:** `npm run bootstrap` (`npx emdash init` + `npx emdash seed`).
- **Rutas de posts:** listado `/posts`, artículo `/posts/:slug` (alinear `PUBLIC_EMDASH_POSTS_PREFIX=/posts` en el sitio marketing).
- **Git:** commits y push desde `blog/` hacia el repo conectado a Cloudflare Pages (`main`).
